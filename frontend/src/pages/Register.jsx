...
import { Signup } from "../services/AuthServices"; // Import Signup from AuthServices
...
const Register = () => {
  ...
  const {
    ...
    initialValues: {
      ...
      password: "",
      phone: "0",
      username: "", // Add username to initial values
      terms: false,
    },
    onSubmit: (values) => {
      if (values.phone.length === 11) {
        Signup(
          ...
          values.password,
          values.phone,
          values.username // Include username in Signup function
        ).then((result) => {
          if (result && result.data && result.data.newUser) {
            navigate("/login");
            toast({
            ...
            toast({
              title: "Error!",
              description:
                result.data?.error?.message || "Registration failed.",
              status: "error",
              duration: 2000,
              isClosable: true,
            });
  ...
  return (
    ...
    >
      <Box width={{ base: "100vw", sm: "500px" }} p={2}>
        ...
        <FormControl mt={3} isInvalid={touched.username && errors.username}>
