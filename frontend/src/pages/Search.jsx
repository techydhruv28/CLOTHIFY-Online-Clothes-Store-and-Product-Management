...
<Box px={{ base: 2, sm: 3, md: 5 }} my={3} py={3} backgroundColor='whitesmoke'>
  <Box width='100%' height='auto' display='flex' justifyContent='space-between' py={{ base: 3, md: 5 }}>
    ...
    <Select colorScheme='facebook' onChange={handleChange} value={sortBy} backgroundColor='#fff' width={{ base: '130px', md: '170px' }}>
      <option value='recommended'>Best Sellers</option>
      <option value='lowest'>Lowest Price</option>
      <option value='highest'>Highest Price</option>
    </Select>
  </Box>
  <SimpleGrid minChildWidth={{ base: '280px', sm: '280px', md: '280px', lg: '280px', xl: '280px' }} gap={3} spacingX={5}>
  <FilterMenu openFilter={openFilter} columns={{ base: 1, sm: 2, md: 2, lg: 3, xl: 4 }} setProducts={setProducts} setSortBy={setSortBy} />
  {products && products.map((product, index) => (
    <Box key={index} width='100%' height='auto' display='flex' justifyContent='center'>
      <ClothesCard productId={product._id} width='100%' height='auto' maxWidth='280px' maxHeight='auto' />
    </Box>
  ))}
  {products.length === 0 && (
    <Box display='flex' justifyContent='start'>
      <Box display='flex' justifyContent='center' alignItems='center' flexDirection='column' mt={10} p={3}>
        ...
        <Button variant='solid' fontSize={20} px={10} mt={10} colorScheme='facebook' onClick={() => navigate('/')}>
          Start Shopping
        </Button>
      </Box>
    </Box>
